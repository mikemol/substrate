...
## Spec build discipline (I1)
`el-atlas-spec.md` is COMPOSED, not hand-edited. Sources live in `spec-src/`
(MANIFEST order; each part sized for a confident one-pass rewrite). Edit a
part, then run `python3 tools/compose-spec.py` — it diffs the proposed
composition against the prior version, previews any shed lines, and refuses
a net shrink without `ALLOW_SHRINK=1`. The composed spec stays committed
(it is the deliverable); part files are the editing surface. Threshold
rule: any working document that outgrows ~20KB gets the same treatment.
(`el-atlas-cotype.md` is exempt — append-only via cat is size-safe;
`el-atlas-structured.md` is generated by its own tool.)
