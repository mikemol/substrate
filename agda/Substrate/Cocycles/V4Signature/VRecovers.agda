...
--   v-for σ                       = v-of-axis-anchor D (applyₛ σ D)      [V.v-for]
--   v-of-axis-anchor D x          = v-of-axis x                          [V, D-case]
--   applyₛ (embed v · embed-S₃ s) D = applyₛ (embed v) (applyₛ (embed-S₃ s) D)  [·]
--   applyₛ (embed-S₃ s) D         = D                                    [embed-S₃-D]
--   applyₛ (embed v) D            = act-axis v D                         [embed.apply = act-axis]
...
open import Substrate.Groups.S4 using (Permutation) renaming (apply to applyₛ)
...
------------------------------------------------------------------------
-- v-recovers. applyₛ (embed v · embed-S₃ s) D reduces:
--   = applyₛ (embed v) (applyₛ (embed-S₃ s) D)   [· composition]
--   = applyₛ (embed v) D                          [embed-S₃-D]
...
v-recovers :
  ...
  proj₁ (perm-to-compositional (compositional-to-perm (v , s))) ≡ v
v-recovers v s =
  trans (cong (λ x → v-of-axis (applyₛ (embed v) x)) (embed-S₃-D s))
        (v-of-axis-act-D v)
