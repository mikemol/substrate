...
--
-- PolyTerm semantics — the UNIQUE fold out of the free tower. With the term
-- carrier routed through the witness tower (PolyTerm = LehmerPath), the
-- denotation of a term is `fold` into any LehmerAlgebra target (base = the
-- identity poly-morphism, step = apply one poly generator), and it is UNIQUE
...
open import Substrate.Category.Poly.Term using (PolyTerm)

-- The denotation: the unique fold into any LehmerAlgebra target.
eval : {C : ℕ → Set} → LehmerAlgebra C → {n : ℕ} → PolyTerm n → C n
eval = fold

-- Its universal property: any interpreter respecting (base, step) IS eval.
eval-unique :
  {C : ℕ → Set} (alg : LehmerAlgebra C) (g : {n : ℕ} → PolyTerm n → C n) →
  (g start ≡ LehmerAlgebra.base alg) →
  (∀ {n} (l : PolyTerm n) (p : Fin (suc n)) → g (l ◂ p) ≡ LehmerAlgebra.step alg (g l) p) →
  ∀ {n} (l : PolyTerm n) → g l ≡ eval alg l
