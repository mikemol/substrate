...
--
-- A SIEVE on an object U is a downward-closed family of UPTermO morphisms into
...
-- UPTermO O Hom. The `member` codomain lowers Set₁ → Set (the Set₁ was only to
...
module _ (O : Set) (Hom : O → O → Set) where
  ...
  record Sieve (U : O) : Set₁ where
    field
      member  : {V : O} → UPTermO O Hom V U → Set
      closure : {V W : O} (t : UPTermO O Hom V U) (u : UPTermO O Hom W V) →
                member t → member (u ++ᵤO t)
