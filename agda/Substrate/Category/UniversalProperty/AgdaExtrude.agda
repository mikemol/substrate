...
import Substrate.Foundation.List.Any as Any
...
------------------------------------------------------------------------
data Tm : Set where      -- ⟦shape:533ef80d S K I,app⟧
  S K I : Tm
  app   : Tm → Tm → Tm
...
------------------------------------------------------------------------
-- membership COMPOSED from the canonical home Foundation.List.Any (was re-derived; the 293 exile reversed by
-- decomposing to the canonical piece, not banishing the module). t ∈ ts = Any (t ≡_) ts; here/there adapt.
_∈_ : Tm → List Tm → Set
t ∈ ts = Any.Any (λ y → t ≡ y) ts
