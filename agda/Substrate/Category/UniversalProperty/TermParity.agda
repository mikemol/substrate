...
-- ⟡ta-upterm: the parity cochain flipped onto the Set₀ O-forms (UPTermO).
open import Substrate.Category.UniversalProperty.Term using (UPTermO; []O; _∷O_; _++ᵤO_)
...
module _ {O : Set} {Hom : O → O → Set} where
  ...
  ----------------------------------------------------------------------
  -- The ℤ/2 cochain: word-length parity. `[]O` is even; each `_∷O_` flips.
  ----------------------------------------------------------------------

  upterm-parity : {X Y : O} → UPTermO O Hom X Y → Bool
  upterm-parity []O      = false
  upterm-parity (_ ∷O t) = not (upterm-parity t)
  ...
  upterm-parity-++ :
    {X Y Z : O} (s : UPTermO O Hom X Y) (t : UPTermO O Hom Y Z) →
    upterm-parity (s ++ᵤO t) ≡ (upterm-parity s xor upterm-parity t)
  upterm-parity-++ []O      t = refl
  upterm-parity-++ (g ∷O s) t =
