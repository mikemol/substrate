...
-- iso. Given an `SFinP.Permutation 3`, produces a `Σ Permutation
-- (Stab anchor)` that fixes anchor and permutes the 3 non-anchor
-- axes according to s.
--
-- The definitions enumerate the 4 anchor × 4 axis = 16 cases. The
-- round-trip proofs use `with ... in p` to capture the SFinA.apply /
-- SFinA.invₐ equation, and helper functions `inv-l-helper` and
-- `inv-r-helper` to chain through SFinA.inv-l / SFinA.inv-r via
...
--   extend-apply anchor (s SFinV.⁻¹) ≡ extend-invₐ anchor s
-- now hold by `refl` parametrically (since SFinV._⁻¹ swaps apply ↔
...
extend-apply : (anchor : Axis) → SFinP.Permutation 3 → Axis → Axis
extend-apply anchor s = extend-fn anchor (SFinA.apply s)

extend-invₐ : (anchor : Axis) → SFinP.Permutation 3 → Axis → Axis
extend-invₐ anchor s = extend-fn anchor (SFinA.invₐ s)
...
extend-stab :
  (anchor : Axis) (s : SFinP.Permutation 3) →
...
-- compose, rewrite the composite to id pointwise (via SFinA.inv-l/-r),
...
-- pushed through by one β-law, then SFinA.inv-l collapses apply∘invₐ.
-- ONE proof, reused at all 12 off-diagonal positions.
off-l :
  (anchor : Axis) (s : SFinP.Permutation 3) (p : Fin 3) →
  extend-fn anchor (SFinA.invₐ s) (fin3-to-non-anchor anchor (SFinA.apply s p))
  ≡ fin3-to-non-anchor anchor p
off-l anchor s p =
  trans (extend-fn-β anchor (SFinA.invₐ s) (SFinA.apply s p))
        (cong (fin3-to-non-anchor anchor) (SFinA.inv-l s p))

off-r :
  (anchor : Axis) (s : SFinP.Permutation 3) (p : Fin 3) →
  extend-fn anchor (SFinA.apply s) (fin3-to-non-anchor anchor (SFinA.invₐ s p))
  ≡ fin3-to-non-anchor anchor p
off-r anchor s p =
  trans (extend-fn-β anchor (SFinA.apply s) (SFinA.invₐ s p))
        (cong (fin3-to-non-anchor anchor) (SFinA.inv-r s p))
...
extend :
  (anchor : Axis) → SFinP.Permutation 3 → Σ Permutation (Stab anchor)
...
------------------------------------------------------------------------
-- Pointwise congruence: if s, t : SFinP.Permutation 3 agree on every
...
extend-apply-pointwise-cong :
  (anchor : Axis) (s t : SFinP.Permutation 3) →
  ((i : Fin 3) → SFinA.apply s i ≡ SFinA.apply t i) →
  (x : Axis) →
  apply (proj₁ (extend anchor s)) x ≡ apply (proj₁ (extend anchor t)) x
