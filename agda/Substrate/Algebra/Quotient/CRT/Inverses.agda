...
-- ≡ 0 mod N (a multiple of N, via mod-mult-hom + suc-mod-suc-self); e₂ mirrors.
--
...
open import Substrate.Algebra.Nat.Mod using (_mod-suc_; suc-mod-suc-self)
...
-- (Ⓓ: the local suc-mod-self re-proved Nat.Mod's suc-mod-suc-self; use it.)
modulus-multiple : (n k : ℕ) → (suc n * k) mod-suc n ≡ 0
modulus-multiple n k =
  trans (mod-mult-hom (suc n) k n)
        (cong (λ z → (z * (k mod-suc n)) mod-suc n) (suc-mod-suc-self n))
