...
------------------------------------------------------------------------
-- HasOrderPerm-compose-commute-order-2 — V₄-shape construction.
--
-- Two commuting order-2 permutations compose to an order-2 permutation.
-- The classical "Klein four-group is generated by two commuting
-- involutions whose product is also an involution" lemma at the
-- permutation level.
--
-- Chain:
--   (σ ∘ τ)² i = σ (τ (σ (τ i)))
--              ≡ σ (σ (τ (τ i)))     [cong σ on commute (τ i), reversed]
--              ≡ σ (σ i)             [cong (σ ∘ σ) on τ-ord i]
--              ≡ i                   [σ-ord i]
--
-- Convention: commute is stated as `σ (τ i) ≡ τ (σ i)` — left-to-right
-- application order, i.e., "σ then τ ≡ τ then σ" when read function-
-- composition style.
--
-- Per [[project-3plus1-parity-universal]]: V₄ ⊂ S₄ acts on F₂² by
-- pairs of commuting involutions; this is the structural lemma that
-- closes V₄ as a permutation group of order-2 elements.
------------------------------------------------------------------------
