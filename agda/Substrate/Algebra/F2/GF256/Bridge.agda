...
import Substrate.Algebra.Polynomial.Graded.Mod.Expand as ModExpand
open import Substrate.Algebra.F2.GF256.Bridge.Base
-- ⟡public-policy: bind at the OWNER — `ytime`/`reduce-mod-f` are Mod.Core's, `hsum` is
-- Mod.Expand's, the rest is Graded's API; none is siphoned through the `Mod` tip.
-- ⟡cap128-bridge-split: the shared kernel (element bridge, lifts, ytime≡xtime, gfold) is
-- `Bridge.Base`; this unit carries only the three ★ transports and their fold schemas.
open ModCore.Over   F₂-CommRing 7 m-lo using (ytime; reduce-mod-f)
open ModExpand.Over F₂-CommRing 7 m-lo renaming (hsum to hsum-g)
open F.Over F₂-CommRing
  using (_+P_; _·c_; x-shift; shift-to-suc-on-left; pad-end)
