...
s₁∘s₂-cubed-on-e₂ :
  apply ((s₁ ∘L s₂) ∘L (s₁ ∘L s₂) ∘L (s₁ ∘L s₂)) (basis ₂)
    ≡ apply id-L (basis ₂)
s₁∘s₂-cubed-on-e₂ =
  cong-trans (apply (s₁ ∘L s₂)) (cong (apply (s₁ ∘L s₂))
              (cong (apply s₁) s₂-on-e₂))
  (cong-trans (apply (s₁ ∘L s₂)) (cong (apply (s₁ ∘L s₂)) s₁-on-e₁)
  (cong-trans (apply (s₁ ∘L s₂)) (cong (apply s₁) s₂-on-e₀)
  (cong-trans (apply (s₁ ∘L s₂)) s₁-on-e₀
  (cong-trans (apply s₁) s₂-on-e₁
         s₁-on-e₂))))
