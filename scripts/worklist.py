...
WORKLIST = [
    ...
    {"sym": "⊙.morton", "desc": "C3 DEEP cell + GENERATOR of the grading cluster. FOUNDATION done ([.idx] CayleyDickson.Cocycle: F₂ⁿ basis map e + index-XOR (zipWith _xor_) + ℂ cocycle). OPEN (the deep arc): the GENERIC product law eᵢ·eⱼ ≈# ε(i,j)·e_{i⊕j} ∀n + explicit ε + its 2-cocycle identity + Morton(trivial ε)-vs-CD(nontrivial). ⟹ C2/C4/C7 [.grade]", "done": ("manual",)},
