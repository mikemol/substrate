...
WORKLIST = [
    ...
    {"sym": "⊙.c3", "desc": "C3 commute-edge bottom rung: mul-comm-ℂ — Cayley-Dickson mul is COMMUTATIVE at level 1 (ℂ), over ≈# (CayleyDickson.CommuteEdge; + the reusable -ℚ-cong). The ladder endpoints now both proved (ℂ comm here, 𝕊 zero-divisor in CayleyDickson). OPEN: ℍ-noncomm/𝕆-nonassoc rungs + the F₂ⁿ Morton/cocycle bridge", "done": ("defines", "Substrate/Algebra/CayleyDickson/CommuteEdge.agda", "mul-comm-ℂ")},
    {"sym": "⊙", "desc": "commuting-sphere: the 8 prove-or-correct GF(2)-VM conjectures — C1 KEYSTONE r<b-residue = the Free⊣Forgetful adjoint correction (⟹ metaphor→proof is prove-or-correct); C2 graded-GF(2) (ungraded was the mistake); C3 Morton≅Cayley-Dickson via the cocycle; C4 F₂ⁿ-index grading; C5 tropical-VM cost; C6 Kirchhoff/conductance; C7 edge=grade=cocycle; C8 Morton/Hilbert curve-crossover at the grade boundary. C1 ✅ ([.c1]) + C6 ✅ ([.c6]) + C3 commute-edge bottom rung ✅ ([.c3] ℂ-comm; ladder endpoints done). REMAINING: C3 cocycle/Morton bridge + ℍ/𝕆 rungs, C2/C4/C7/C8 — MANUAL until each lands. Ledger: scratch/commuting_sphere.md; in-code index Algebra.Wedge.Sphere",
     "done": ("manual",)},
