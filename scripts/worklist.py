...
WORKLIST = [
    ...
    {"sym": "⊙.morton", "desc": '✅ C3 PROVED — the GENERIC PRODUCT LAW (CayleyDickson.Cocycle.prod): eᵢ·eⱼ ≈# ε(i,j)·e_{i⊕j} for ALL n, with the explicit recursive 2-cochain ε. Built bottom-up: [.idx] basis-map e + index-XOR · [.setoid] ≈#-equivalence · [.conj] conj-on-basis · [.cong] ≈#-congruences · [.unit] zero-family + CD identity + normalization (ε(i,0)=+1) · [.neg] negation-distribution · [.prod] the 4-case index-XOR induction. Morton = trivial cochain (ε≡false, pure XOR); CD = this nontrivial ε (noncomm = mul-noncomm-ℍ, strict-2-cocycle failure = the 𝕆 associator). Grounds C7 (edge=grade=cocycle); C2/C4 [.grade] remain', "done": ("defines", "Substrate/Algebra/CayleyDickson/Cocycle.agda", "prod")},
    {"sym": "⊙.assoc", "desc": "✅ the ASSOCIATOR as a measured 3-cochain (CayleyDickson.Cocycle) — the ⊙.measure instance turning prod's unclaimed 𝕆-associator into a tracked obstruction. dε = the coboundary 3-cochain of ε; associator proves (eᵢ·eⱼ)·eₖ ≈# dε(i,j,k)·(eᵢ·(eⱼ·eₖ)) (both land at e_{i⊕j⊕k}, dε is the sign difference); associative-at: dε=false ⟹ associates; dε-𝕆 ≡ true recovers mul-nonassoc-𝕆 from the cochain. Unifies #3 (cocycle) + #5 (graded obstruction): nonassociativity = ε's non-closure, graded", "done": ("defines", "Substrate/Algebra/CayleyDickson/Cocycle.agda", "associator")},
    {"sym": "⊙.pentagon", "desc": '✅ the PENTAGON — dε is a CLOSED 3-cochain (CayleyDickson.Cocycle), discharging the G8 suspicion (the one unverified claim in the closed sphere). δ²/δ³ = the F₂ⁿ group-cohomology coboundary operators; dε-is-coboundary proves dε ≡ δ²ε (dε IS the coboundary of ε, hence closed since coboundaries are closed/d²=0); pentagon-𝕆₁₂₃ verify δ³(dε) ≡ false at octonion 4-tuples where dε itself is nonzero — the associator obstruction is COHERENT (higher cell closes). Fully-generic d²=0 ∀φ (a 20-term xor-AC cancellation, each face-pair via ⊕-assoc + xor-same) is the remaining cohomological refinement', "done": ("defines", "Substrate/Algebra/CayleyDickson/Cocycle.agda", "dε-is-coboundary")},
    {"sym": "⊙.dd", "desc": '✅ generic d²=0 ∀φ (CayleyDickson.Coboundary.d²-zero) — makes ⊙.pentagon UNCONDITIONAL. δ³(δ²φ) ≡ false for EVERY 2-cochain φ (not just the 𝕆 witnesses): the 20-term face expansion has each of 10 distinct values twice (simplicial pairing), so it cancels over F₂. Proved via a foldrB xor-parity framework (foldrB-++ homomorphism + remove-head-pair) — flatten the tree to a list, cancel the 10 pairs head-first; the two diagonal faces aligned by ⊕-assoc. So dε = δ²ε is closed for ALL cochains', "done": ("defines", "Substrate/Algebra/CayleyDickson/Coboundary.agda", "d²-zero")},
    ...
    {"sym": "⊙.morton.neg", "desc": "the NEG-DISTRIBUTION family (CayleyDickson.Setoid): neg-invol (−−x=x), conj-neg (conj∘neg=neg∘conj), neg-add (−(x+y)=−x+−y), sub-neg, and mul-negˡ/ʳ (mul distributes negation through either factor, mutually via the doubling formula). The last foundation cluster — needed to push conj-on-basis's ± sign through the product in the imaginary×imaginary cases. ℚ bases via mkℚ-pattern + ≈ℚ-from-≡(subst)", "done": ("defines", "Substrate/Algebra/CayleyDickson/Setoid.agda", "mul-negʳ")},
    {"sym": "⊙.morton.prod", "desc": 'THE THEOREM: prod (CayleyDickson.Cocycle) — eᵢ·eⱼ ≈# ε(i,j)·e_{i⊕j} ∀n by induction on the level; each of the 4 doubling cases reduces via mul-zero (off-diagonal halves) + conj-on-basis + mul-neg (push the ± sign) + the IH to the signed XOR basis unit. Plus the explicit ε, ⊕-comm (XOR commutes), and the sgn-threading helpers (sgn-cons/zero#/cong, mul-sgn-l/r, neg-sgn, sgn-xor). Capstone of ⊙.morton', "done": ("defines", "Substrate/Algebra/CayleyDickson/Cocycle.agda", "prod")},
    {"sym": "⊙.grade", "desc": "✅ C4 + C2 PROVED (CayleyDickson.Grade). The F₂ⁿ index carries TWO gradings: the LENGTH (Wedge.VecGraded.vec-graded Bool — the generic length-grading at Bool, reused not reinvented; = the ambient F₂ⁿ dimension) and POPCOUNT (the ANF degree = Hamming weight) — they don't coincide, both grade the one index. C4: the two index-PRODUCTS (CD/Morton XOR vs ANF/Reed-Muller OR-union) bridged by ∨-anf (OR=XOR⊕AND). C2: popcount-or-and (popcount(i∨j)+popcount(i∧j)=popcount i+popcount j) — the cancellation defect IS the overlap popcount(i∧j), zero iff disjoint supports; graded index retains it, only the ungraded popcount projection loses it. C7 grounded by ⊙.morton.prod", "done": ("defines", "Substrate/Algebra/CayleyDickson/Grade.agda", "popcount-or-and")},
    {"sym": "⊙.c8", "desc": "✅ GROUNDED (framing, CayleyDickson.Curve) — the Morton/Hilbert curve-crossover as a tropical (cost) instance. morton = the Z-order linearization (F₂ⁿ index → rank, reusing Grade.bit); crossover IS the tropical min ⊓ (NatInf, the cost gauge ⊙.c6's Contraction reads) — the chosen curve is the argmin, the boundary is where it flips (concrete Morton-cheaper-low / Hilbert-wins-high). Full Hilbert construction (recursive quadrant reflection) is a separate build; only the SELECTION mechanism is C8's content", "done": ("defines", "Substrate/Algebra/CayleyDickson/Curve.agda", "crossover")},
    {"sym": "⊙.hilbert", "desc": "✅ the HILBERT curve built — the last ⊙.measure instance, de-degenerating crossover (CayleyDickson.Curve). gray = the binary-reflected (Gray) code = the 1D Hilbert/reflected-binary curve (recursive reflection, b_i↦b_i⊕b_{i+1}); gray-roundtrip proves it's a bijection (faithful linearization like morton). LOCALITY is now real: morton-jump≡2 vs gray-jump≡1 on the 01→10 step (popcount of the actual curve steps), and crossover-gray-wins feeds those to the tropical min (Gray wins) — costs no longer hand-picked fin. Full 2D Hilbert (quadrant ROTATION) is the remaining refinement", "done": ("defines", "Substrate/Algebra/CayleyDickson/Curve.agda", "gray-roundtrip")},
    {"sym": "⊙.hilbert2d", "desc": '✅ the full 2D HILBERT curve (reflection + ROTATION) — CayleyDickson.Hilbert2D, resolving the Gray=Hilbert wedge (gray = 1D reflection-only corner; this adds the quadrant rotation 2D locality needs). hilbert : ℕ→List(ℕ×ℕ), recursive: 4 transformed copies/level (A transpose + D anti-diagonal reflect = the rotations, B/C translate). hilbert-1 ≡ the U tetromino; hilbert-2-continuous (maxjump ≡ 1 across ALL quadrant seams) validates the construction by refl — a wrong rotation would jump >1. ∀n continuity (inductive entry/exit-adjacency invariant) is the remaining refinement', "done": ("defines", "Substrate/Algebra/CayleyDickson/Hilbert2D.agda", "hilbert-2-continuous")},
    {"sym": "⊙.hilbert∀n.entry", "desc": '🔶 PARTIAL — the ∀n ENTRY invariant (CayleyDickson.Hilbert2DContinuous.hilbert-entry): the 2D Hilbert curve starts at (0,0) for EVERY order n (via hilbert-head: hilbert n = (0,0)∷…, A=transpose fixes the origin) + the firstP/lastP ++/mapL scaffolding. SCOPED (~100 lines): the EXIT recursion (lastP = (2ⁿ−1,0) — lastP-peeling through the 4 anon-lambda transforms; a refactor naming A/B/C/D would un-fiddle it), a coord-BOUND invariant, the 4 Manhattan-ISOMETRIES (D needs bounded-monus (k∸a)∸(k∸b)≡b∸a), and Cont-++ seam assembly. Lowest-priority remaining item; concrete hilbert-1/2-continuous already verify it', "done": ("defines", "Substrate/Algebra/CayleyDickson/Hilbert2DContinuous.agda", "hilbert-entry")},
    {"sym": "⊙.c5", "desc": '✅ C5 PROVED (Wedge.Wedderburn) — nilpotent-not-annihilating ⟹ no dead-end. Annihilator (kills EVERY product, Wedge.Mul) separated from Nilpotent (kills only own powers): e₁₂ is a nonzero nilpotent (e₁₂²=𝟎) yet e₁₂-not-annihilating (e₁₂·e₂₁=e₁₁≠𝟎 — the corner glues back to a live idempotent). The obstruction is graded by degree, never a collapse; ties #1 (the wedge correction always survives)', "done": ("defines", "Substrate/Algebra/Wedge/Wedderburn.agda", "e₁₂-not-annihilating")},
    {"sym": "⊙.c5b", "desc": "✅ the GRADED-COST half of #5 + ⊙.measure instance (Wedge.CrossMulGraded) — a CrossMix Two→M₂ whose cross-term nilpotency degree GENUINELY VARIES, replacing coherent-everywhere's all-degree-0 (two-mul square-zero). Embeds ε↦e₁₁ (left)/e₁₂ (right): coherent-graded gives (ε,ε)↦e₁₁·e₁₂=e₁₂ degree-2 (n=1) vs others↦𝟎 degree-0 (n=0); cross-εε-not-clean proves the (ε,ε) term is a NONZERO obstruction (can't be witnessed clean). M₂ is the 'richer common carrier' CrossMul's comment flagged as needed", "done": ("defines", "Substrate/Algebra/Wedge/CrossMulGraded.agda", "coherent-graded")},
    {"sym": "⊙", "desc": 'commuting-sphere: the 8 prove-or-correct GF(2)-VM conjectures (authoritative ledger + live index: Algebra.Wedge.Sphere; prose: scratch/commuting_sphere.md). ALL 8 now machine-checked & live-referenced: #1 KEYSTONE r<b=Free⊣Forgetful ✅ Wedge.Adjunction [.c1] · #2 graded-GF(2) non-cancelling ✅ CayleyDickson.Grade [.grade] · #3 Morton≅CD cocycle ✅ Cocycle.prod [.morton] · #4 two gradings ✅ Grade [.grade] · #5 nilpotent≠annihilating ✅ Wedderburn [.c5] · #6 semiring-VM gauge ✅ Semiring.Contraction [.c6] · #7 edge=grade=cocycle ✅ grounded by prod · #8 Morton/Hilbert crossover ✅ Curve [.c8] + Gray [.hilbert] + 2D-rotation [.hilbert2d]. PLUS the ⊙.measure family: associator/dε [.assoc], graded-degree CrossMix [.c5b], pentagon dε-closed [.pentagon]. The arc is CLOSED end-to-end; remaining = DEEP refinements (not conjectures): generic d²=0 ∀φ, ∀n Hilbert continuity', "done": ("all", [("defines", "Substrate/Algebra/Wedge/Sphere.agda", "cell-cocycle"), ("defines", "Substrate/Algebra/Wedge/Sphere.agda", "cell-pentagon")])},
