...
def deserialize_from_projection(con):
    ...
    pt = {p: t for p, t in QB.run(con, "pathtext_all")}
    tt = {i: t for i, t in QB.run(con, "terms_all")}
    units = {u: (pt.get(np), tt.get(k), pt.get(mp), rl)                 # unit_id -> (qname,kind,module,root_lid)
             for u, np, k, mp, rl in QB.run(con, "unit_fields")}
    cod = {u: (tt.get(cc) if cc else None, pt.get(cq) if cq else None)  # unit_id -> codomain (ctor,qname)
           for u, cc, cq in QB.run(con, "unit_cod")}
    members = collections.defaultdict(list)
    for su, o, mnp, muid in QB.run(con, "unit_member"):
        members[su].append((pt.get(mnp), muid))
    refs = collections.defaultdict(set)
    for u, rq in QB.run(con, "deserialize_oppath"):
        refs[u].add(rq)
...
def _failed(con):
    row = QB.run(con, "meta_failed").fetchone()
...
def render_index(con):
    ...
    structs = sorted(((r[0], r[1], r[2], r[3] or "")
                      for r in QB.run(con, "structs_index")),
                     key=lambda x: (x[0].lower(), x[2]))
    namehomes = {}
    for name, module in QB.run(con, "structs_namemod"):
        namehomes.setdefault(name, set()).add(module)
    fingerprints = {}
    for fp, name, module in QB.run(con, "structs_fpnamemod"):
        fingerprints.setdefault(fp, []).append((name, module))
    idxmods = [(m, p or "") for m, p in
               QB.run(con, "modules_index")]
...
def render_graph(con):
    failed = _failed(con)
    struct_kind = dict(QB.run(con, "structs_qnamekind").fetchall())
    edges = {(x, y) for x, y in QB.run(con, "edges_all")}
...
def render_sitemap(con):
    ...
    rows = QB.run(con, "sitemap").fetchall()
...
def render_usage(con):
    ...
    total = QB.run(con, "structs_count").fetchone()[0]
    refined_in  = {r[0] for r in QB.run(con, "edges_dst_distinct")}
    refines_out = {r[0] for r in QB.run(con, "edges_src_distinct")}
    allq = [r[0] for r in QB.run(con, "structs_qname")]
    ...
    top = QB.run(con, "indegree_top").fetchall()
    ...
    out += ["", f"## Structurally isolated ({len(isolated)}) — neither refine nor are refined\n",
            "| structure | kind | home |", "|---|---|---|"]
    kinds = dict(QB.run(con, "structs_qnamekind").fetchall())
    for q in isolated:
...
def render_import(con):
    ...
    failed = _failed(con)
    edges = {(x, y) for x, y in QB.run(con, "modedges_all")}
