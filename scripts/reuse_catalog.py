...
# The relational store (Ⓓ.catalog-db): the SAME facts the two markdown catalogs render, as a
# queryable sqlite DB, so a NEW discovery question is a SELECT rather than a new script. The
# markdown files stay the committed, diffable, LLM-consumable renders; the .db is a derived local
# cache (non-deterministic binary → gitignored, regenerated by the gate / on demand). The `fp`
# (shape fingerprint) is computed in Python — where the member-name sort is already correct —
# rather than in SQL, so it matches the reuse-index fingerprint exactly without relying on a
# recent SQLite's ordered GROUP_CONCAT.
_DB_SCHEMA = """
CREATE TABLE structs (qname TEXT PRIMARY KEY, name TEXT, kind TEXT, module TEXT, fp TEXT);
...
-- The two catalogs' key queries, as views (the "new question = a SELECT" payoff):
...
# ─────────────────────────── reducer 3: catalog-DB (relational store) ───────────────────────────
class DbBuilder:
    def __init__(self):
        self.structs, self.members, self.refrows = [], [], []
        self.kinds, self.refs = {}, {}

    def add(self, core):
        mod, nodes = core["mod"], core["nodes"]
        for unit, root, kind, members in core["defmarks"]:
            if kind not in ("Datatype", "Record"):
                continue
            ...
            self.structs.append((unit, unit.rsplit(".", 1)[-1], kd, mod, fp))
    ...
    def write(self, failed=0):
        ...
        def owner(qn):                         # longest structure-prefix (== GraphBuilder's rule)
            parts = qn.split(".")
            for j in range(len(parts), 0, -1):
                p = ".".join(parts[:j])
                if p in structs:
...
def generate(filt="", do_index=True, do_graph=True, do_db=False):
    ...
    db = DbBuilder()    if do_db    else None
    failed = 0
