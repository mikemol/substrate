...
# ---------------------------------------------------------------------------
# Skeleton construction — substitute per-file residue tokens with a hole
# marker, then check whether the files reduce to a single parametric form.
#
# Where the recursive template REPORTS shared structure level-by-level,
# skeleton construction CONSTRUCTS a usable artefact: it tokenizes each
# file, identifies tokens that vary across files (the residue), and
# substitutes them with a hole marker. If all files reduce to identical
# text afterward, that text IS the parametric template — the holes' map
# is the per-file parameter list.
#
# For pure orbits (e.g., post-rename Z_n-x-FreeCyclic), all files reduce
# to the same template. For partial orbits (e.g., Coxeter cores), some
# divergence remains; we show the largest aligned subset.
# ---------------------------------------------------------------------------
...
def construct_skeleton(
    ...
    *,
    ...
    if all_match:
        ...
        for p in paths:
            tokens = sorted(residue_per_file[p])
            print(f"# {p.name}:")
            for t in tokens:
    ...
    # Compute line-level cross-file intersection on the substituted texts.
    ...
    # Print the first file's substituted text, marking shared lines.
    ...
    for p in paths:
        tokens = sorted(residue_per_file[p])
        print(f"# {p.name} ({len(tokens)} residue tokens):")
        for t in tokens[:max_show]:
            print(f"#   {hole_marker}  ←  {t}")
        if len(tokens) > max_show:
            print(f"#   ... ({len(tokens) - max_show} more)")
