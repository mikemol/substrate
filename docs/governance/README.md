...
| [scoped-classical-hypotheses.md](scoped-classical-hypotheses.md) | Classical logic (LEM) enters only as a scoped, visible hypothesis — never a postulate; weakest principle, with a constructive discharge where possible | Direct (module parameters under `--safe`; model at `ConjugationCoalgebra.Simplicity`) |
