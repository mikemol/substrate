#!/usr/bin/env python3
"""query_defs.py — ⟡qinfra: the COMPILER. The SINGLE SOURCE for the project's SQL read-queries as named
SQLAlchemy-Core `select()` builders. This module imports SQLAlchemy (a ~0.4s cost); the CACHED RUNTIME is the
sibling `query_builders.py`, which imports NO SQLAlchemy and executes precompiled SQL from the generated
`query_sql.py` by NAME. So the hot paths (sppf_db `--ingest-core`, the pre-commit gates) `import query_builders`
and pay ~nothing; only this compiler + `query_registry.py` (which needs the builder OBJECTS to intern) touch
SQLAlchemy. The builder-NAME is its INTERN_BUILDERS key (`q_event_head` ↔ `"event_head"`).

  `python3 query_defs.py --compile`  → (re)generate query_sql.py (`SQL = {name: (sql, default_params)}`)
  `python3 query_defs.py --check`    → drift gate: recompile in-memory, diff vs the committed query_sql.py
  `python3 query_defs.py --verify`   → migration gate: each builder's rows == its ORIGINAL raw SQL over catalog.db
...
def q_reuse_rows(has_copy, limit=None):  # sppf_query reuse_rows L86-96 (DYNAMIC → conditional builder)
    ...
    if limit is not None:
        stmt = stmt.limit(limit)                         # int OR a bindparam (the two static variants below)
    return stmt

# ⟡q-reuse-rows-copy / ⟡q-reuse-rows-nocopy — the DYNAMIC sppf_query:71 `q_reuse_rows(has_copy(c), limit)`
# resolved into two STATIC named builders (the runtime picks by name, no SQLAlchemy). `limit` is a bindparam:
# pass lim=-1 for "no limit" (SQLite LIMIT -1 = unbounded ≡ the original no-LIMIT clause). VERIFIED: q_fanin
# already binds `LIMIT :lim` under sqlite3, so a parameterized LIMIT is safe.
def q_reuse_rows_copy():   return q_reuse_rows(True,  bindparam("lim"))
def q_reuse_rows_nocopy(): return q_reuse_rows(False, bindparam("lim"))
...
def _reg():
    return {
        ...
        "reuse_rows_copy": (q_reuse_rows_copy, {"min_units": 3, "lim": -1},
            ...
            "GROUP BY un.node_id HAVING units>=? ORDER BY units DESC LIMIT ?", (3, -1)),
        "reuse_rows_nocopy": (q_reuse_rows_nocopy, {"min_units": 3, "lim": -1},
            "SELECT un.node_id, COALESCE(NULLIF(pt.text,''), o.text, r.text, '?') head, "
            "COUNT(DISTINCT un.unit_id) units, NULL copies FROM unit_node un "
            "JOIN _node nd ON nd.node_id=un.node_id JOIN unit u ON u.unit_id=un.unit_id "
            "LEFT JOIN path_text pt ON pt.path_id=nd.op_path_id LEFT JOIN terms o ON o.term_id=nd.op_term_id "
            "LEFT JOIN terms r ON r.term_id=nd.role_id WHERE nd.node_id IN (SELECT node_id FROM node_child) "
            "GROUP BY un.node_id HAVING units>=? ORDER BY units DESC LIMIT ?", (3, -1)),
...
# ── the compiled-query cache: query_sql.py (⟡qinfra) ────────────────────────────────────────────────
# `--compile` renders every INTERN_BUILDERS entry to its SQLite SQL + its baked default params; the runtime
# (query_builders.py) executes SQL[name] with NO SQLAlchemy. `--check` recompiles in-memory and diffs vs the
# committed file (drift → exit 1), same anti-drift discipline as gen_build_makefiles --check.
QUERY_SQL = os.path.join(HERE, "query_sql.py")

def _compile_one(stmt):
    compiled = stmt.compile(dialect=sqlite.dialect(paramstyle="named"))
    return str(compiled), {k: compiled.params[k] for k in sorted(compiled.params)}

def compile_text():
    """Render query_sql.py source — deterministic (builders + param keys sorted)."""
    rows = []
    for name in sorted(INTERN_BUILDERS):
        sql, params = _compile_one(INTERN_BUILDERS[name]())
        rows.append(f"  {name!r}: ({sql!r}, {params!r}),")
    return ("# GENERATED by query_defs.py --compile — DO NOT EDIT BY HAND.\n"
...
def do_compile():
    with open(QUERY_SQL, "w") as f:
        f.write(compile_text())
    print(f"query_defs --compile: wrote {QUERY_SQL} ({len(INTERN_BUILDERS)} builders)")

def do_check():
    fresh = compile_text()
    try:
        on_disk = open(QUERY_SQL).read()
    except FileNotFoundError:
        print(f"query_defs --check: {QUERY_SQL} MISSING — run --compile", file=sys.stderr); return False
    if fresh == on_disk:
        print(f"query_defs --check: query_sql.py fresh ({len(INTERN_BUILDERS)} builders)"); return True
    print("query_defs --check: query_sql.py STALE — run `python3 jea/metalanguage/query_defs.py --compile`",
          file=sys.stderr)
    return False
...
def main():
    ...
    ap.add_argument("--compile", action="store_true", help="(re)generate query_sql.py from the builders")
    ap.add_argument("--check", action="store_true", help="drift gate: assert query_sql.py matches the builders")
