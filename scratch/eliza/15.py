"""15.py: power-signature ranking + branched predictions via spectral reflection.

Adds two things on top of 14.py:

  * POWER SIGNATURE panel: chambers ranked by sqrt(Σ φ_k(x)²) over the low-
    frequency macro modes (1..5). Static — it's the manifold's intrinsic
    spectral energy distribution. Complements the visit-count ranking.

  * BRANCHES panel: at every step, take the recent chamber trajectory, lift
    to spectral (Fiedler, turbulence) coords, compute derivatives, flip
    sign on each axis subset, reintegrate, reproject to chambers. Yields
    three shadow trajectories (Φ̄, T̄, Φ̄T̄) alongside the actual — a 4-way
    branched prediction set, each deterministic but reflecting different
    chirality choices on the recent rhythm.

Usage:
    python 15.py                        # interactive
    python 15.py path/to/file.txt       # stream the file
    python 15.py file.txt --rate 30
    python 15.py --smoke                # no-UI smoke check
"""
...
# --- Manifold + holonomy: identical to 14.py. ---
...
# --- Spectral analytics: power signatures + reflected (shadow) trajectories. ---


class SpectralAnalytics:
    """Per-chamber spectral power, and axis-flipped shadow trajectories.

    Power signature uses low-frequency macro modes (1..5 by default), so each
    chamber gets a meaningful scalar (modes-1..23 norm would be constant by
    Parseval).

    Shadow trajectory: given a chamber sequence, lift to spectral coords in
    `reflection_modes` (default Fiedler + turbulence), compute derivatives,
    sign-flip per axis, reintegrate from the first point, reproject each
    integrated coordinate to the nearest chamber.
    """

    def __init__(self, manifold,
                 power_modes=(1, 2, 3, 4, 5),
                 reflection_modes=(1, 2)):
        self.M = manifold
        self.power_modes = tuple(power_modes)
        self.reflection_modes = tuple(reflection_modes)
        self.phi_power = np.asarray(manifold.eigenvectors[:, list(power_modes)])
        self.phi_refl = np.asarray(manifold.eigenvectors[:, list(reflection_modes)])
        # Precompute the per-chamber power signature.
        norms = np.linalg.norm(self.phi_power, axis=1)
        self._powers_by_word = {
            _word_str(manifold.shortlex_paths[x]): float(norms[i])
            for i, x in enumerate(manifold.nodes_list)
        }

    def powers_by_word(self):
        return dict(self._powers_by_word)

    def shadow_trajectory(self, trajectory, sign_flips):
        """trajectory: list of chamber tuples.
        sign_flips: tuple of ±1, one per reflection_mode.
        Returns: list of chamber tuples of the same length.
        """
        if len(trajectory) < 2:
            return list(trajectory)
        idxs = [self.M.nodes_list.index(x) for x in trajectory]
...
# --- Trigram predictor: same as 14.py. ---
...
# --- Engine: walker + predictor + reflection trajectory. ---


TRAJECTORY_WINDOW = 24
SHADOW_SIGN_FLIPS = [
    ("Φ̄",   (-1, +1)),   # Fiedler-flipped
    ("T̄",   (+1, -1)),   # Turbulence-flipped
    ("Φ̄T̄", (-1, -1)),   # Both
]
...
class Engine:
    ...
    def shadow_trajectories(self):
        """Return {label: [chamber_word_str, ...]} for each non-identity flip."""
...
def _build_app(engine, source=None, rate=20.0):
    ...
    class PowerPanel(Static):
        visits = reactive({})
        lengths = reactive({})
        L_MAX = 6  # diameter of S4 Cayley graph on simple reflections

        def render(self):
            if not self.lengths:
                return "[bold]Top by power[/bold]\n[dim]initialising[/dim]"
            # Power signature: sqrt((L/L_max)² + (V/V_max)²). Normalising each
            # axis to [0,1] before L2-combining keeps depth and frequency
            # comparable; without it, visit counts (often 20..100) dominate
            # length (0..6) and the ranking collapses to top-by-visits.
    ...
    class SynthesisPanel(Static):
        ...
        def render(self):
            ...
            return f"[bold]Synthesis[/bold]   [dim](sampled continuation)[/dim]\n[green]{shown}[/green]"
    ...
    class ElizaApp(App):
        ...
        #synthesis { height: 4; border: round $accent; padding: 0 1; }
        ...
        def on_mount(self):
            ...
            # Chain lengths are static per chamber; set once at mount.
            lengths = {
                _word_str(self.engine.manifold.shortlex_paths[x]): len(self.engine.manifold.shortlex_paths[x])
...
def main(argv):
    ...
    if args.smoke:
        ...
        lengths = {
            _word_str(engine.manifold.shortlex_paths[x]): len(engine.manifold.shortlex_paths[x])
            for x in engine.manifold.nodes_list
        }
        scores = {
            w: (l * l + visits.get(w, 0) ** 2) ** 0.5
            for w, l in lengths.items()
        }
        top_power = sorted(scores.items(), key=lambda kv: -kv[1])[:6]
        shadows = engine.shadow_trajectories()
        print(f"SMOKE OK; chamber={_word_str(engine.current_physics['shortlex'])}")
        print("top by power (length² + visits²):")
        for w, p in top_power:
            print(f"  {w:>14}  power={p:5.2f}  L={lengths[w]}  V={visits.get(w, 0)}")
        print("trajectory tail (last 7):")
        tail = [_word_str(engine.manifold.shortlex_paths[x]) for x in engine.trajectory][-7:]
        print(f"  actual: {' → '.join(tail)}")
        for label, traj in shadows.items():
            print(f"  {label}: {' → '.join(traj[-7:])}")
        engine.end()
        return 0
