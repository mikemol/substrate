"""17.py: V₄-orbit decomposition + chirality + period detection.

Three substrate-inspired additions on top of 16.py:

  * ORBIT panel (replaces Power): the 24 chambers decompose into 6
    V₄-cosets × 4 fiber positions. Per Substrate.Cocycles.V4Signature,
    the orbit (= Pairing × Chirality) is the invariant; the within-orbit
    position is gauge-equivalent. The panel sums visits across each
    coset's 4 fibers and shows the 6 invariants instead of 24 chambers.

  * CHIRALITY badge in ChamberPanel: parity of the current shortlex word
    (= parity of the permutation = bipartite-mode sign). Each Coxeter
    step flips it. Colour-coded.

  * PERIOD detection: smallest k such that the chamber k steps back
    equals the current chamber, scanned from the trajectory deque.
    Per Substrate.Category.Coalgebra.FiniteOrder, this is the local
    'HasOrder γ k' witness — the Z/k cyclic subgroup acting on the
    recent trajectory.

Everything else carries over from 16.py: sqlite persistence, branched
syntheses, trigram predictor, holonomy, sparklines.

Usage:
    python 17.py                        # interactive
    python 17.py path/to/file.txt       # stream the file
    python 17.py file.txt --rate 30     # stream at 30 chars/sec
    python 17.py --smoke                # no-UI smoke check
"""
...
# V₄ ≤ S₄ as four permutation tuples: e, (12)(34), (13)(24), (14)(23).
# Per Substrate.Cocycles.V4Signature: invariant = orbit of right action
# (chamber · V₄); 24 chambers = 6 cosets × 4 fiber positions.
V4_PERMS = (
    (1, 2, 3, 4),  # e
    (2, 1, 4, 3),  # α = (12)(34)
    (3, 4, 1, 2),  # β = (13)(24)
...
# --- Manifold + holonomy + analytics: identical to 16.py + orbit decomp. ---
...
class SpectralManifold:
    ...
    def _compute_v4_orbits(self):
        # For each chamber x, build the right V₄-coset (chamber · V₄), pick the
        # shortlex-min element as canonical, and record (orbit, chirality, fiber)
        # per chamber. Chirality = parity of shortlex word length.
        self.orbit_of = {}      # chamber -> canonical chamber of its V₄-coset
        self.chirality_of = {}  # chamber -> "even" / "odd"
        self.fiber_of = {}      # chamber -> V₄-element label
        for chamber in self.nodes_list:
            coset = [_perm_compose(chamber, v) for v in V4_PERMS]
            canonical = min(
                coset,
                key=lambda c: (len(self.shortlex_paths[c]), self.shortlex_paths[c]),
            )
            self.orbit_of[chamber] = canonical
            self.chirality_of[chamber] = (
                "even" if len(self.shortlex_paths[chamber]) % 2 == 0 else "odd"
            )
            v = _perm_compose(_perm_inverse(canonical), chamber)
            self.fiber_of[chamber] = V4_LABELS[V4_PERMS.index(v)]
...
class Engine:
    ...
    def detect_period(self):
        # Smallest k > 0 such that trajectory[-1-k] == current. Captures the
        # local HasOrder γ k witness — k=2 for involution echoes (same gen
        # applied twice), small k for tight cycles, None when diffusive.
        traj = list(self.trajectory)
        if len(traj) < 2:
            return None
        current = traj[-1]
    ...
    def walk_text_orbits(self, text, start_chamber=None):
        # For each char in `text`, walk the chamber via char_to_generator and
        # record the resulting orbit (V₄-coset canonical word). Used to colour
        # the stream view and synthesis text by orbit.
        chamber = start_chamber if start_chamber is not None else self.current_chamber
        out = []
        for ch in text:
            chamber = self.manifold.apply_reflection(chamber, char_to_generator(ch))
            orbit_word = _word_str(self.manifold.shortlex_paths[self.manifold.orbit_of[chamber]])
            out.append((ch, orbit_word))
        return out
...
def _build_orbit_palette(manifold):
    # 6 V₄-cosets mapped to 6 distinct colours: cool tones for even chirality,
    # warm for odd. Stable across runs (sorted by chirality then shortlex).
    orbits = sorted(
        set(manifold.orbit_of[c] for c in manifold.nodes_list),
        key=lambda c: (
            manifold.chirality_of[c],
            len(manifold.shortlex_paths[c]),
            manifold.shortlex_paths[c],
        ),
    )
    ...
    odd_i = 0
    for o in orbits:
        ow = _word_str(manifold.shortlex_paths[o])
        if manifold.chirality_of[o] == "even":
            palette[ow] = cool[even_i % len(cool)]
            even_i += 1
        else:
            palette[ow] = warm[odd_i % len(warm)]
            odd_i += 1
    return palette


def _colored(seq, palette, default="white"):
    # seq: iterable of (display_char, orbit_word). Returns Rich markup string.
    out = []
    for ch, orbit in seq:
        color = palette.get(orbit, default)
        # Escape brackets in the char to avoid breaking markup.
        safe = ch.replace("[", "\\[")
        out.append(f"[{color}]{safe}[/]")
    return "".join(out)
...
def _build_app(engine, source=None, rate=20.0):
    ...
    PALETTE = _build_orbit_palette(engine.manifold)
    ...
    class ChamberPanel(Static):
        ...
        fiber = reactive("e")
        period = reactive(0)  # 0 means no period detected in window

        def render(self):
            chir_color = "cyan" if self.chirality == "even" else "magenta"
            ...
            return (
                ...
                f"[{chir_color}]{chir_glyph} {self.chirality}[/{chir_color}]  "
    ...
    class OrbitsPanel(Static):
        visits = reactive({})
        # word → canonical word for its V₄-coset; word → "even"/"odd"
        orbit_of = reactive({})
        ...
        def render(self):
            ...
            # Sum visits across each V₄-coset's 4 fiber chambers.
            orbit_visits = {}
            for chamber_word, count in self.visits.items():
                orbit_word = self.orbit_of.get(chamber_word, chamber_word)
                orbit_visits[orbit_word] = orbit_visits.get(orbit_word, 0) + count
            for orbit_word in set(self.orbit_of.values()):
                orbit_visits.setdefault(orbit_word, 0)
            ...
            for orbit_word, count in ranked:
                chir = self.chirality_of.get(orbit_word, "even")
                chir_glyph = "+" if chir == "even" else "−"
                color = PALETTE.get(orbit_word, "white")
                bar = "█" * max(1, int(14 * count / top))
                lines.append(
                    f"[{color}]{chir_glyph} {orbit_word:>16} {bar}[/{color}] {count}"
                )
            return "\n".join(lines)
    ...
    class SynthesisPanel(Static):
        # actual: list[(display_char, orbit_word)]
        # branches: dict[label, list[(display_char, orbit_word)]]
        actual = reactive([])
        branches = reactive({})
        n_obs = reactive(0)
        ...
        def render(self):
            ...
            lines = ["[bold]Synthesis[/bold]   [dim]chars coloured by V₄-coset visited[/dim]"]
            lines.append(f"[bold]actual[/bold]  {_colored(self.actual, PALETTE)}")
            for label, seq in self.branches.items():
                lines.append(f"[bold]{label:>6}[/bold]  {_colored(seq, PALETTE)}")
            return "\n".join(lines)
    ...
    class ElizaApp(App):
        ...
        def on_mount(self):
            ...
            # Push static orbit decomposition into the OrbitsPanel.
            M = self.engine.manifold
            orbit_of_word = {
                _word_str(M.shortlex_paths[c]):
                    _word_str(M.shortlex_paths[M.orbit_of[c]])
                for c in M.nodes_list
            }
            chirality_of_word = {
                _word_str(M.shortlex_paths[c]): M.chirality_of[c]
                for c in M.nodes_list
            }
            orbits_panel = self.query_one("#orbits", OrbitsPanel)
            orbits_panel.orbit_of = orbit_of_word
            orbits_panel.chirality_of = chirality_of_word
        ...
        def _refresh_synthesis(self):
            ...
            actual_text = self.engine.predictor.project_text()
            panel.actual = self._colorize(actual_text)
            panel.branches = {
                label: self._colorize(text)
                for label, text in self.engine.synthesize_branches().items()
            }
            panel.n_obs = self.engine.predictor.n_observations()

        def _colorize(self, text):
            # Walk text from current chamber, get orbit per step, build the
            # (display_char, orbit_word) list the panel renders.
            display = []
            for ch, orbit in self.engine.walk_text_orbits(text):
                d = ch if ch.isprintable() and ch != "\n" else ("↵" if ch == "\n" else "·")
                display.append((d, orbit))
            return display
        ...
        def _consume(self, ch):
            ...
            display_ch = ch if ch.isprintable() and ch != "\n" else ("↵" if ch == "\n" else "·")
            current_orbit_word = _word_str(
                self.engine.manifold.shortlex_paths[
                    self.engine.manifold.orbit_of[self.engine.current_chamber]
                ]
            )
            self.stream_buf.append((display_ch, current_orbit_word))

            chamber_panel = self.query_one("#chamber", ChamberPanel)
            chamber_panel.word = result["to_word"]
            chamber_panel.coord = str(phys["coordinate"])
            chamber_panel.chirality = self.engine.manifold.chirality_of[self.engine.current_chamber]
            chamber_panel.fiber = self.engine.manifold.fiber_of[self.engine.current_chamber]
            period = self.engine.detect_period()
            chamber_panel.period = period if period is not None else 0
            ...
            self.query_one("#orbits", OrbitsPanel).visits = visits_dict
        ...
        def _refresh_chamber(self):
            ...
            chamber.chirality = self.engine.manifold.chirality_of[self.engine.current_chamber]
            chamber.fiber = self.engine.manifold.fiber_of[self.engine.current_chamber]
            period = self.engine.detect_period()
            chamber.period = period if period is not None else 0
    ...
    try:
        if args.smoke:
            ...
            print(f"SMOKE OK; chamber={_word_str(engine.current_physics['shortlex'])} "
                  f"({M.chirality_of[here]}, fiber {M.fiber_of[here]})")
            print(f"period: {engine.detect_period()}")
            # 6-orbit summary.
            visits = dict(recorder.aggregate["chamber_visits"]) if recorder else {}
            orbit_sums = {}
            for c in M.nodes_list:
                w = _word_str(M.shortlex_paths[c])
                orbit_w = _word_str(M.shortlex_paths[M.orbit_of[c]])
                orbit_sums[orbit_w] = orbit_sums.get(orbit_w, 0) + visits.get(w, 0)
            print("orbits (V₄-cosets):")
            for orbit_w, count in sorted(orbit_sums.items(), key=lambda kv: -kv[1]):
                chir = M.chirality_of[M.orbit_of[
                    next(c for c in M.nodes_list if _word_str(M.shortlex_paths[c]) == orbit_w)
                ]]
                print(f"  {chir:>4} {orbit_w:>20}  visits={count}")
